...
func (d *OpenAIReq) GetModel(l *LLM) {
	...
	switch utils.GetTxtType(llmConfigRaw) {
...
func GenerateOpenAIImg(ctx context.Context, prompt string, imageContent []byte) ([]byte, int, error) {
	...
	start := time.Now()
	var llmConfig *param.LLMConfig
	userInfo := db.GetCtxUserInfo(ctx)
	if userInfo != nil && userInfo.LLMConfigRaw != nil {
		llmConfig = userInfo.LLMConfigRaw
	}
	mediaType := utils.GetImgType(llmConfig)
	var imgModel string
	if llmConfig != nil {
		imgModel = llmConfig.ImgModel
	}
	model := utils.GetUsingImgModel(mediaType, imgModel)
...
func OpenAITTS(ctx context.Context, content, encoding string) ([]byte, int, int, error) {
	...
	userInfo := db.GetCtxUserInfo(ctx)
	if userInfo != nil && userInfo.LLMConfigRaw != nil {
		ttsModel = userInfo.LLMConfigRaw.TTSModel
	}
	model := utils.GetUsingTTSModel(param.OpenAi, ttsModel)
	metrics.APIRequestCount.WithLabelValues(model).Inc()
...
func GetOpenAIClient(ctx context.Context, clientType string) *openai.Client {
	...
	switch clientType {
	case "txt":
		t = utils.GetTxtType(llmConfigRaw)
	case "img":
		t = utils.GetImgType(llmConfigRaw)
	case "video":
		t = utils.GetVideoType(llmConfigRaw)
	case "rec":
		t = utils.GetRecType(llmConfigRaw)
	}
