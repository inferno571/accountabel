...
func CreateBot(ctx context.Context) *tgbotapi.BotAPI {
	...
	cmdCfg := tgbotapi.NewSetMyCommands(
		...
		tgbotapi.BotCommand{
			Command:     param.Setup,
			Description: "Set daily check-in time (e.g. /setup 20:30)",
		},
		tgbotapi.BotCommand{
			Command:     param.Log,
			Description: "Log craving level 1-10 (e.g. /log 3)",
		},
		tgbotapi.BotCommand{
			Command:     param.StreakCmd,
			Description: "View your current sobriety streak",
		},
		tgbotapi.BotCommand{
			Command:     param.LinkCmd,
			Description: "Link onboarding web preferences (e.g. /link 123456)",
		},
	)
	bot.Send(cmdCfg)
