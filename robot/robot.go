...
func (r *RobotInfo) GetAudioContent(audioContent []byte) (string, error) {
	...
	default:
		err = fmt.Errorf("unsupported audio recognition type: %s", t)
	}
...
func (r *RobotInfo) ExecCmd(cmd string, defaultFunc func(), modeFunc func(string), typesFunc func(string)) {
	...
	case param.CronClear, "/" + param.CronClear, "$" + param.CronClear:
		r.cronClear()
	case param.Setup, "/" + param.Setup, "$" + param.Setup:
		r.handleSetup()
	case param.Log, "/" + param.Log, "$" + param.Log:
		r.handleLog()
	case param.StreakCmd, "/" + param.StreakCmd, "$" + param.StreakCmd:
		r.handleStreak()
	case param.LinkCmd, "/" + param.LinkCmd, "$" + param.LinkCmd:
		r.handleLink()
	case "start", "/start", "$start":
		args := strings.TrimSpace(r.Robot.getPrompt())
		if len(args) == 6 {
			r.handleLink()
		} else {
			r.sendHelpInfo()
...
func (r *RobotInfo) ExecLLM(msgContent string, msgChan *MsgChan) {
	...
	// CRISIS INTERCEPTOR: Check for crisis keywords BEFORE calling the LLM.
	// If detected, bypass the LLM entirely and return hardcoded crisis resources.
	if CheckCrisisKeywords(content) {
		logger.WarnCtx(r.Ctx, "crisis keyword detected, bypassing LLM", "userID", userId)
		if msgChan.NormalMessageChan != nil {
			msgChan.NormalMessageChan <- &param.MsgInfo{
...
func (r *RobotInfo) CreatePhoto(prompt string, lastImageContent []byte) ([]byte, int, error) {
	return nil, 0, fmt.Errorf("image generation is not supported in this build")
}

func (r *RobotInfo) CreateVideo(prompt string, lastImageContent []byte) ([]byte, int, error) {
	return nil, 0, fmt.Errorf("video generation is not supported in this build")
}
